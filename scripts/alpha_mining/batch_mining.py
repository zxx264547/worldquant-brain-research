...
class AlphaResult:
    ...
    ppc: float
    margin: float
    status: str  # 'ready', 'need_optimize', 'rejected'
    ...
    def is_submittable(self) -> bool:
        """检查是否满足提交条件"""
        checks = {
            'sharpe': self.sharpe >= 1.58,
            'fitness': self.fitness > 0.5,
            'ppc': self.ppc < 0.5,
            'margin_gt_turnover': self.margin > self.turnover,
            'turnover': self.turnover > 0.01,
        }
