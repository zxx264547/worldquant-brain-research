---
name: wq-onboarding
description: "WorldQuant BRAIN 项目启动配置：环境搭建、BRAIN 账号配置、认证验证、认知循环启动顺序。克隆项目后首次使用或新 AI Agent 接入时执行。"
whenToUse: "新环境/新会话首次启动量化研究项目时，需要配置 Python 环境、BRAIN 凭据或验证认证时使用。"
---
...
如果要用 `seed_alpha_generator.py`（论文驱动 Alpha 生成），设置环境变量：

```bash
export WQ_AI_API_KEY="sk-你的密钥"   # AI 服务密钥（DeepSeek 等）
```

> 注意：凭据禁止硬编码在脚本中，统一从 `user_config.json` 或环境变量读取。
...
### Alpha 提交 201 但不生效
必须用 `curl -L` 跟踪 303 重定向（详见 `wq-alpha-submit` skill）：
```bash
