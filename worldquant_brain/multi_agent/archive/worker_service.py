...
from worldquant_brain.multi_agent.expression_validator import validate_expression
...
class WorkerService:
    ...
    def _setup_logger(self):
        ...
        return get_logger(f"worker_{self.worker_num}",
                          str(LOG_DIR / f"{self.worker_id}.log"))
    ...
    def _init_brain_client(self):
        ...
        if self.brain_client is None:
            ...
            self.logger.info("BRAIN API client initialized")

    def run_daemon(self, poll_interval: int = 10):
        """Daemon模式：持续监听并处理任务"""
        self.logger.info(f"{self.worker_id} daemon started (poll_interval={poll_interval}s)")
        ...
        while True:
            try:
                ...
                if ideas:
                    self.logger.info(f"Found {len(ideas)} ideas to process")
                    for idea in ideas:
                        self._process_idea(idea)
                else:
                    self.logger.info("No ideas assigned, sleeping...")
                ...
                self.logger.info(f"ERROR in daemon loop: {e}")
                import traceback
                traceback.print_exc()
                time.sleep(poll_interval)
    ...
    def run_once(self):
        """单次执行模式：处理一次任务后退出"""
        self.logger.info(f"{self.worker_id} single run started")
        ...
        try:
            ...
            if ideas:
                ...
                self.logger.info("No ideas assigned")
    ...
    def _get_assigned_ideas(self) -> List[dict]:
        ...
        try:
            ...
            self.logger.info(f"Error loading ideas: {e}")
            return []
    ...
    def _process_idea(self, idea: dict):
        ...
        self.logger.info(f"Processing idea {idea_id}: field={field}, op={operator}, window={window}")
        ...
        try:
            ...
            self.logger.info(f"Expression: {expr}")

            # 验证表达式
            is_valid, errors, warnings = validate_expression(expr)
            if warnings:
                for w in warnings:
                    self.logger.info(f"  Warning: {w}")
            if not is_valid:
                self.logger.info(f"  Expression validation failed: {errors}")
                self.logger.info("  Skipping simulation and marking as failed")
            ...
            self.logger.info(f"ERROR processing idea {idea_id}: {e}")
            self._update_idea_status(idea_id, 'failed')
            import traceback
            traceback.print_exc()
    ...
    def _run_simulation(self, expr: str, idea: dict) -> dict:
        ...
        self.logger.info(f"Running simulation: {expr[:80]}...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"Result type: {type(result).__name__}, keys: {list(result.keys()) if isinstance(result, dict) else 'N/A'}")
                ...
                self.logger.info(f"Simulation completed: sharpe={sharpe}, fitness={fitness}")
                ...
                if '429' in error_str or 'Too Many Requests' in error_str:
                    self.logger.info(f"Rate limited, waiting {retry_delay}s before retry ({attempt+1}/{max_retries})...")
    ...
    def _wait_for_simulation(self, sim_id: str, timeout: int = 900) -> str:
        ...
        while time.time() - start < timeout:
            try:
                ...
                self.logger.info(f"Check {sim_id}: {state}")
                ...
                self.logger.info(f"Error checking status: {e}")
                time.sleep(check_interval)
    ...
    def _save_result(self, idea_id: int, result: dict):
        ...
        try:
            ...
            self.logger.info(f"Result saved for idea {idea_id}")

        except Exception as e:
            self.logger.info(f"Error saving result: {e}")
    ...
    def _publish_result(self, idea_id: int, result: dict):
        ...
        self.logger.info(f"Result event published for idea {idea_id}")
    ...
    def _update_idea_status(self, idea_id: int, status: str):
        ...
        try:
            ...
            self.logger.info(f"Error updating idea status: {e}")
